...
if (isDev) {
  document.getElementById("settingsOfflineAccountTypeContainer").style.display =
    "";
}
...
function processLogOut(val, isLastAccount) {
  ...
  if (targetAcc.type === "microsoft") {
    ...
    const removeAccountAction = ConfigManager.isOfflineAccount(targetAcc)
      ? AuthManager.removeOfflineAccount(uuid)
      : AuthManager.removeMojangAccount(uuid);

    removeAccountAction.then(() => {
...
const settingsCurrentMojangAccounts = document.getElementById(
  "settingsCurrentMojangAccounts",
);
const settingsCurrentOfflineAccounts = document.getElementById(
  "settingsCurrentOfflineAccounts",
);

function resolveAuthAccountImage(acc, size = 60) {
  if (ConfigManager.isOfflineAccount(acc)) {
    return "assets/images/SealCircle.png";
  }
  return `https://mc-heads.net/body/${acc.uuid}/${size}`;
}
  ...
  authKeys.forEach((val) => {
    ...
    const accHtml = `<div class="settingsAuthAccount" uuid="${acc.uuid}">
            <div class="settingsAuthAccountLeft">
                <img class="settingsAuthAccountImage" alt="${acc.displayName}" src="${resolveAuthAccountImage(acc)}">
            </div>
    ...
    } else if (ConfigManager.isOfflineAccount(acc)) {
