...
async function ensureDevOfflineAccount() {
  if (!isDev || Object.keys(ConfigManager.getAuthAccounts()).length > 0) {
    return null;
  }
  ...
  try {
    ...
    console.error("Unable to create development offline account.", err);
    return null;
  }
}

async function showMainUI(data) {
  const devOfflineAccount = await ensureDevOfflineAccount();
  ...
  setTimeout(() => {
    ...
    document.body.style.backgroundImage =
      "url('assets/images/backgrounds/bg2.png')";
    $("#main").show();
    ...
    if (
      ...
      !ConfigManager.isOfflineAccount(ConfigManager.getSelectedAccount())
    ) {
