...
const OFFLINE_ACCOUNT_TYPE = "offline";
...
function resolveDefaultOfflineUsername() {
  ...
  try {
    ...
    return (
      process.env.USERNAME ?? process.env.USER ?? OFFLINE_USERNAME_FALLBACK
    );
  }
}
...
function sanitizeOfflineUsername(username) {
  const resolvedUsername = (
    typeof username === "string" ? username : resolveDefaultOfflineUsername()
  )
    .replace(OFFLINE_USERNAME_SANITIZER, "")
    .slice(0, 16);

  return resolvedUsername.length > 0
    ? resolvedUsername
    : OFFLINE_USERNAME_FALLBACK;
}
...
function purgeDevOnlyAccounts() {
  ...
  authKeys.forEach((uuid) => {
    if (config.authenticationDatabase[uuid]?.type === OFFLINE_ACCOUNT_TYPE) {
      delete config.authenticationDatabase[uuid];
      removedAny = true;
      removedSelected = removedSelected || config.selectedAccount === uuid;
    }
  });
  ...
  if (
    removedAny &&
    (removedSelected ||
      config.authenticationDatabase[config.selectedAccount] == null)
  ) {
    const remainingKeys = Object.keys(config.authenticationDatabase);
    if (remainingKeys.length > 0) {
      config.selectedAccount = remainingKeys[0];
    } else {
 ...
 */
exports.addOfflineAuthAccount = function (
  username = resolveDefaultOfflineUsername(),
) {
  const resolvedUsername = sanitizeOfflineUsername(username);
  const uuid = createOfflineUUID(resolvedUsername);

  config.selectedAccount = uuid;
  config.authenticationDatabase[uuid] = {
    type: OFFLINE_ACCOUNT_TYPE,
...
/**
 * Determine if the given account is a local offline account.
 *
 * @param {Object} authAccount The account to check.
 * @returns {boolean} True if the account is offline.
 */
exports.isOfflineAccount = function (authAccount) {
  return authAccount != null && authAccount.type === OFFLINE_ACCOUNT_TYPE;
};
